...
export async function exportAll() {
  const jobs = await listJobs();
  const [
    profile, resume, comp, samples, models, usage,
    linkedInProfileUrl, email, phone, location,
    workLocations, workPreferences, knowledgeBase,
    prepQuestions, negativeKeywords, savedSearches,
    autoAnalyzeHighMatch, localModel,
  ] = await Promise.all([
    getProfile(), getMasterResume(), getCompTargets(), getWritingSamples(), getModels(), getUsage(),
    getLinkedInProfileUrl(), getContactEmail(), getContactPhone(), getContactLocation(),
    getWorkLocations(), getWorkPreferences(), getKnowledgeBase(),
    getPrepQuestions(), getNegativeKeywords(), getSavedSearches(),
    getAutoAnalyzeEnabled(), getLocalModelSettings(),
  ]);
  return {
    exportVersion: 1,
    exportedAt: new Date().toISOString(),
    settings: {
      profile, masterResume: resume, compTargets: comp, writingSamples: samples, models,
      linkedInProfileUrl, email, phone, location,
      workLocations, workPreferences, knowledgeBase,
      prepQuestions, negativeKeywords, savedSearches,
      autoAnalyzeHighMatch, localModel,
...
export async function importAll(data) {
  ...
  if (typeof s.linkedInProfileUrl === 'string') await setLinkedInProfileUrl(s.linkedInProfileUrl);
  if (typeof s.email === 'string') await setContactEmail(s.email);
  if (typeof s.phone === 'string') await setContactPhone(s.phone);
  if (typeof s.location === 'string') await setContactLocation(s.location);
  if (Array.isArray(s.workLocations)) await setWorkLocations(s.workLocations);
  if (s.workPreferences && typeof s.workPreferences === 'object') await setWorkPreferences(s.workPreferences);
  if (typeof s.knowledgeBase === 'string') await setKnowledgeBase(s.knowledgeBase);
  if (Array.isArray(s.prepQuestions)) await setPrepQuestions(s.prepQuestions);
  if (Array.isArray(s.negativeKeywords)) await setNegativeKeywords(s.negativeKeywords);
  if (Array.isArray(s.savedSearches)) await chrome.storage.local.set({ [KEY_SAVED_SEARCHES]: s.savedSearches });
  if (typeof s.autoAnalyzeHighMatch === 'boolean') await setAutoAnalyzeEnabled(s.autoAnalyzeHighMatch);
  if (s.localModel && typeof s.localModel === 'object') await setLocalModelSettings(s.localModel);
